...
Graphics::Graphics() {
	// Inicia variáveis membros
	swapChain = nullptr;						// Ponteiro para SwapChain
	renderTargetView = nullptr;					// Render Target View
	blendState = nullptr;						// Mistura de cores
	featureLevel = D3D_FEATURE_LEVEL_11_0;		// Versão de Direct3D

	bgColor[0] = 0.0f;							// Red
	bgColor[1] = 0.0f;							// Green
	bgColor[2] = 0.0f;							// Blue
	bgColor[3] = 0.0f;							// Alpha
...
#endif
		// Cria dispositivo para acessar Direct3D
		if FAILED(
			D3D11CreateDevice(
